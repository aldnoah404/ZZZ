import torch
from model import U_NeXt_v1, U_NeXt_v2,U_NeXt_v4
...
def model_info_test(model_name="U_NeXt_v1"):
    ...
    for name, param in model.named_parameters():
        if param.requires_grad:
            prefix = name.split('.')[0]
            layer_params[prefix] += param.numel()
    
    for prefix, param_count in layer_params.items():
        print(f"层前缀: {prefix} \t 参数数量: {param_count}")
...
if __name__ == "__main__":
    # model_name = "U_NeXt_v2"
    # model_name = "SSLSE"
    # model_name = "U_NeXt_v4"
    model_name = "SegNet"
    model_info_test(model_name=model_name)